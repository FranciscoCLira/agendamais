...
function Start-Container {
    ...
    $exists = (docker ps -a --format "{{.Names}}" | Select-String -Pattern "^$containerName$") -ne $null
    if ($exists) {
        Write-Host "Removing existing container $containerName"
        docker rm -f $containerName | Out-Null
    }
    docker run -d --name $containerName -e POSTGRES_USER=$pgUser -e POSTGRES_PASSWORD=$pgPassword -e POSTGRES_DB=$pgDb -p $pgPort:5432 postgres:15 | Out-Null
...
function Run-Flyway-Migrate {
    ...
    $mvnCmd = "mvn -DskipTests org.flywaydb:flyway-maven-plugin:9.16.0:migrate -Dflyway.url=$($env:SPRING_DATASOURCE_URL) -Dflyway.user=$($env:SPRING_DATASOURCE_USERNAME) -Dflyway.password=$($env:SPRING_DATASOURCE_PASSWORD) -Dflyway.locations=classpath:db/migration"
    Write-Host $mvnCmd
    Invoke-Expression $mvnCmd
    if ($LASTEXITCODE -ne 0) { throw "Flyway migrate failed (exit $LASTEXITCODE)" }
}
...
switch ($Action) {
    ...
    'stop' { Stop-Container }
    'status' { docker ps -a --filter "name=$containerName" }
}
