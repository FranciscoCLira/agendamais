...
public class EstatisticasFixedController {
    ...
    public String estatisticaUsuariosGraficos(Model model, HttpSession session, RedirectAttributes redirectAttributes,
        ...
        if (usuarioLogado == null || nivelAcesso == null || instituicaoSelecionada == null) {
            redirectAttributes.addFlashAttribute("mensagemErro", "Sessão inválida ou expirada.");
        ...
        // Serializar listas ordenadas como JSON no formato [{key, value}, ...]
        String paisesOrdenadosJson = "[]";
        String estadosOrdenadosJson = "[]";
        String cidadesOrdenadasJson = "[]";
        try {
            ...
            System.out.println("*** ERRO ao serializar listas ordenadas para JSON: " + e.getMessage());
        }
        model.addAttribute("paisesOrdenadosJson", paisesOrdenadosJson);
        model.addAttribute("estadosOrdenadosJson", estadosOrdenadosJson);
        model.addAttribute("cidadesOrdenadasJson", cidadesOrdenadasJson);

        return "gestao-usuarios/estatistica-usuarios-graficos";
    }
    ...
    private String processarEstatisticasComSessao(List<UsuarioInstituicao> usuarios, Model model, Usuario usuarioLogado,
        ...
        try {
            ...
            for (UsuarioInstituicao ui : usuarios) {
                if (ui.getUsuario() != null && ui.getUsuario().getPessoa() != null) {
                    ...
                    List<PessoaSubInstituicao> subInstituicoesPessoa = pessoaSubInstituicaoRepository
        ...
        try {
            ...
            System.out
                    .println("*** DEBUG: Processando sub-instituições para instituição ID: " + instituicaoId + " ***");
            ...
            System.out.println("*** DEBUG: Estatísticas por sub-instituição (instituição " + instituicaoId + "): "
                    + usuariosPorSubInstituicao + " ***");
            ...
            long contadorSubInstituicoesAtivas = usuariosPorSubInstituicao.entrySet().stream()
                    ...
                    .count();
            System.out
                    .println("*** DEBUG CONTAGEM: Sub-Instituições Ativas = " + contadorSubInstituicoesAtivas + " ***");
                        ...
                        if (cidade != null) {
                            ...
                            if (estado != null) {
                                ...
                                if (pais != null) {
                                    ...
                                    // País/Estado
                                    String chaveEstado = nomePais + " / " + nomeEstado;
                                    usuariosPorEstado.put(chaveEstado,
                                            usuariosPorEstado.getOrDefault(chaveEstado, 0L) + 1);

                                    // País/Estado/Cidade
                                    String chaveCidade = nomePais + " / " + nomeEstado + " / " + nomeCidade;
                                    usuariosPorCidade.put(chaveCidade,
                                            usuariosPorCidade.getOrDefault(chaveCidade, 0L) + 1);
                                    ...
                                    estadosPorPais.computeIfAbsent(nomePais, k -> new HashMap<>())
                                            .put(nomeEstado,
                                                    estadosPorPais.get(nomePais).getOrDefault(nomeEstado, 0L) + 1);

                                    // Cidades por estado (com chave País/Estado)
                                    cidadesPorEstado.computeIfAbsent(chaveEstado, k -> new HashMap<>())
                                            .put(nomeCidade,
                                                    cidadesPorEstado.get(chaveEstado).getOrDefault(nomeCidade, 0L) + 1);
                            ...
                            usuariosPorEstado.put("Não informado / Não informado",
                                    usuariosPorEstado.getOrDefault("Não informado / Não informado", 0L) + 1);
                            usuariosPorCidade.put("Não informado / Não informado / Não informado",
                                    usuariosPorCidade.getOrDefault("Não informado / Não informado / Não informado", 0L)
                                            + 1);
                        }
                    } else {
                        usuariosPorPais.put("Dados incompletos",
                                usuariosPorPais.getOrDefault("Dados incompletos", 0L) + 1);
                        usuariosPorEstado.put("Dados incompletos / Dados incompletos",
                                usuariosPorEstado.getOrDefault("Dados incompletos / Dados incompletos", 0L) + 1);
                        usuariosPorCidade.put("Dados incompletos / Dados incompletos / Dados incompletos",
                                usuariosPorCidade.getOrDefault(
                                        "Dados incompletos / Dados incompletos / Dados incompletos", 0L) + 1);
                    }
                } catch (Exception e) {
        ...
        System.out.println("*** DEBUG: Processamento concluído - Países válidos: " + totalPaises + " (total com todos: "
                + usuariosPorPais.size() + ")");
     ...
     */
    private String processarEstatisticas(List<UsuarioInstituicao> usuarios, Model model, Long instituicaoId,
        ...
        try {
            for (UsuarioInstituicao ui : usuarios) {
                if (ui.getUsuario() != null && ui.getUsuario().getPessoa() != null) {
                    ...
                    // Verificar se a pessoa tem vínculos com sub-instituições usando o repositório
                    // real
                    boolean temSubInstituicao = pessoaSubInstituicaoRepository.findFirstByPessoaId(pessoaId)
                            .isPresent();
            ...
            System.out.println("*** DEBUG SUB-INSTITUIÇÕES (sem sessão): Com = " + usuariosComSubInstituicoes
                    + ", Sem = " + usuariosSemSubInstituicoes + " ***");
        ...
        // === ESTATÍSTICAS POR SUB-INSTITUIÇÃO (sem sessão - filtrado por instituição
        // ID=1) ===
        Map<String, Long> usuariosPorSubInstituicao = new HashMap<>();

        try {
            Long instituicaoIdFiltro = 1L; // Usar instituição ID=1 para debug
            System.out.println("*** DEBUG: Processando sub-instituições para instituição ID: " + instituicaoIdFiltro
                    + " (modo debug) ***");
            ...
            System.out.println("*** DEBUG: Estatísticas por sub-instituição (instituição " + instituicaoIdFiltro + "): "
                    + usuariosPorSubInstituicao + " ***");
            ...
            long contadorSubInstituicoesAtivas = usuariosPorSubInstituicao.entrySet().stream()
                    .filter(entry -> !entry.getKey().equals("Sem Sub-Instituição") &&
                            !entry.getKey().equals("Erro no processamento"))
                    .count();
            System.out.println("*** DEBUG CONTAGEM (sem sessão): Sub-Instituições Ativas = "
                    + contadorSubInstituicoesAtivas + " ***");
        ...
        System.out.println("*** DEBUG: Países válidos encontrados: " + totalPaises + " (total com todos: "
                + usuariosPorPais.size() + ")");
        ...
        // Ordenar País, Estado, Cidade por percentual (maior para menor)
        List<Map.Entry<String, Long>> paisesOrdenados = new ArrayList<>(usuariosPorPais.entrySet());
        List<Map.Entry<String, Long>> estadosOrdenados = new ArrayList<>(usuariosPorEstado.entrySet());
        List<Map.Entry<String, Long>> cidadesOrdenadas = new ArrayList<>(usuariosPorCidade.entrySet());
    ...
    public String listaEstatisticaPublica(Model model, HttpSession session) {
        ...
        try {
            ...
            System.out.println("*** LISTA ESTATÍSTICA: usuarioLogado = "
                    + (usuarioLogado != null ? usuarioLogado.getUsername() : "NULL"));
            System.out.println("*** LISTA ESTATÍSTICA: instituicaoSelecionada = "
                    + (instituicaoSelecionada != null ? instituicaoSelecionada.getNomeInstituicao() : "NULL"));
    ...
    private String criarDadosSimulados(Model model) {
        ...
        System.out.println(
                "*** DADOS SIMULADOS: Brasil com " + ((List<?>) brasil.get("subNiveis")).size() + " estados ***");
    ...
    public String listaEstatisticaAdministrativa(Model model, HttpSession session) {
        ...
        try {
            ...
            System.out.println(
                    "*** ADMINISTRATIVO: Usando dados REAIS - " + instituicaoSelecionada.getNomeInstituicao() + " ***");
    ...
    private void processarEstatisticasHierarquicas(List<UsuarioInstituicao> usuarios, Model model,
        ...
        String nomeInst = (instituicaoSelecionada != null) ? instituicaoSelecionada.getNomeInstituicao()
                : "Instituição Padrão";
        ...
        System.out.println("*** ESTATÍSTICAS SIMPLIFICADAS: " + paisesEstatisticas.size() + " países, " + totalUsuarios
                + " usuários ***");
    }
    ...
    public String listaEstatisticaUsuarios(Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        try {
            ...
            System.out.println("*** DEBUG LISTA ESTATÍSTICA: usuarioLogado = "
                    + (usuarioLogado != null ? usuarioLogado.getUsername() : "NULL"));
            System.out.println("*** DEBUG LISTA ESTATÍSTICA: nivelAcesso = " + nivelAcesso);
            System.out.println("*** DEBUG LISTA ESTATÍSTICA: instituicaoSelecionada = "
                    + (instituicaoSelecionada != null ? instituicaoSelecionada.getNomeInstituicao() : "NULL"));
            ...
            if (usuarioLogado == null) {
                ...
                System.out.println(
                        "*** MODO DEBUG: Dados simulados criados com " + paisesSimulados.size() + " países ***");
             ...
             * if (nivelAcesso < 5) {
             * System.out.
             * println("*** DEBUG LISTA ESTATÍSTICA: Nível insuficiente - apenas super usuários ***"
             * );
             * redirectAttributes.addFlashAttribute("mensagemErro",
             * "Apenas super usuários podem acessar estatísticas.");
             * return "redirect:/administrador";
             * }
             */
            ...
            if (instituicaoSelecionada == null) {
                ...
                redirectAttributes.addFlashAttribute("mensagemErro",
                        "Nenhuma instituição selecionada para gerar estatísticas.");
                return "redirect:/administrador";
            }
            ...
            // Processar estatísticas usando método existente mas retornando template
            // diferente
